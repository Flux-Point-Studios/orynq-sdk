...
import { sha256Hex, sleep, jitterMs, isMissingFile } from "./util.js";
...
export class OpenClawRecorder {
  ...
  async runForever() {
    ...
    try {
      ...
      if (!isMissingFile(err)) {
        ...
        console.error(`[anchor] schedule unreadable, starting a fresh cycle: ${msg}`);
      }
      nextAnchorAt = 0;
    }
    if (!nextAnchorAt) {
  ...
  async anchorLatestBundles() {
    ...
    for (const f of files.filter((x) => x.endsWith(".jsonl"))) {
      ...
      try {
        ...
        if (prior?.contentDigest === contentDigest && prior?.state === "submitted") {
          ...
          if (requestId) {
            ...
            if (action === "promote-anchored") {
              // The poll is the only path to "anchored" in production (the POST
              // never answers CONFIRMED), so the receipt is rewritten here or it
              // reads "submitted" forever.
              const confirmedAt = Date.now();
              const submitted = (prior.lastReceipt ?? {}) as Record<string, unknown>;
              const polledTxHash = "txHash" in poll ? poll.txHash : undefined;
              const confirmedReceipt = {
                ...submitted,
                anchored: true,
                state: "anchored",
                txHash: polledTxHash ?? submitted.txHash ?? null,
                confirmedAt: new Date(confirmedAt).toISOString()
              };
              await this.writeReceipt(bundleId, confirmedReceipt);
              anchorState[bundleId] = {
                ...prior,
                state: "anchored",
                confirmedAt,
                lastReceipt: confirmedReceipt
              };
        ...
        await this.writeReceipt(bundleId, receipt);
        ...
        // One bundle's local failure (an unreadable spool, a full disk) must
        // not stop the others: state is untouched, so it is retried next cycle.
        const msg = err instanceof Error ? err.message : String(err);
  ...
  private async writeReceipt(bundleId: string, receipt: Record<string, unknown>) {
